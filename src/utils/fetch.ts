...
export const fetchSSE = async (fetchFn: () => Promise<Response>, options: FetchSSEOptions = {}) => {
  ...
  if (!response.ok) {
    // 555 定义为 OpenAI 错误
    const isOpenAIError = response.status === OPENAI_SERVICE_ERROR_CODE;

    const chatMessageError: ChatMessageError = isOpenAIError
      ? {
          type: 'openai',
          message: JSON.parse(response.statusText).message,
